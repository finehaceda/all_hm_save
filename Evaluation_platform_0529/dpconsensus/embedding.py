...
from dpconsensus.config import dpconsensus_path, current_directory
# 将 DNA 序列映射到独热编码
...
def bsalign_ali(cluster_seqs,num):
    ...
    shell = './bsalign-master/bsalign poa '+dpconsensus_path+'/seqs.fasta -o '+dpconsensus_path+'/consus.txt -L > '+dpconsensus_path+'/ali.ali'
...
def getfirstindex(seq1,seq2):
    ...
    shell = './bsalign-master/bsalign align '+dpconsensus_path+'/errors1.fasta > '+dpconsensus_path+'/alierrors1.ali'
    result = subprocess.run(shell, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
...
def bsalign_alitest11(cluster_seqs,num):
    ...
    save_seqs(cluster_seqs, dpconsensus_path+'/seqs.fasta')
    # save_seqs_special(cluster_seqs, 'seqs.fasta')
    shell = current_directory+'/bsalign-master/bsalign poa '+dpconsensus_path+'/seqs.fasta -o '+dpconsensus_path+'/consus.txt -L > '+dpconsensus_path+'/ali.ali'
    # shell = '../bsalign poa one_cluster.fasta -o consus.txt -L>consensus.bsalign'
    result = subprocess.run(shell, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
    # print(result)
    # sleep(300)
...
def read_alifilestest0215(path,num):
    seq_inf = []
    with open(path, "r") as file:
        lines = file.readlines()
    for i in range(2,num+2):
...
def bsalign_alitest_0215(cluster_seqs,num):
    # 0925 hm改，为了得到bsalign的质量值
    save_seqs(cluster_seqs, dpconsensus_path+'/seqs.fasta')
    shell = current_directory+f'/bsalign-master/bsalign poa {dpconsensus_path}/seqs.fasta -o {dpconsensus_path}/consus.txt -L > {dpconsensus_path}/ali.ali'
    result = subprocess.run(shell, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
    # sleep(300)
    # print(f'bsalign shell:{shell}\nrun bsalign result:{result.stderr}')
    seqs,bsquas,aliconsus = read_alifilestest0215(f'{dpconsensus_path}/ali.ali', num)
    bsquas = quality_scores_to_probabilities(bsquas)
    return seqs,aliconsus,bsquas
...
def getRadomSeqsNoQua(dna_sequences,select_nums=5):
    ...
    # random.seed(2045)
    random.seed()
    seeds = random.sample(range(1000), 1)
    print(f"seeds:{seeds}")
    new_dna_sequences=[]
    allseqsnum = 0
    minnum = 1000
...
def trans_seq_hot_nophred(dna_sequences,ori_dna_sequences,copy_num=10,sequence_length = 128):
    ...
    all_ali_seqs_formaxlen = sorted(all_ali_seqs_formaxlen)
    sequence_length = all_ali_seqs_formaxlen[-1]
    isine = -2
    while sequence_length > len(ori_dna_sequences[0]) * 3 and isine > -len(all_ali_seqs_formaxlen):
        sequence_length = all_ali_seqs_formaxlen[isine]
        isine -= 1
...
def trans_seq_hottest(dna_sequences,ori_all_quas,copy_num=10):
    ...
    print('testdata done!')
    return dna_sequences,all_base_proportions,all_base_proportions_phreds,all_consus

def trans_seq_hottest_nophred(dna_sequences,copy_num=10):
    dna_sequences = getRadomSeqsNoQua(dna_sequences,copy_num)
    # current_directory = os.getcwd()
    print("当前工作目录:", current_directory)
    print("dpconsensus_path:", dpconsensus_path)
    save_seqs(dna_sequences,dpconsensus_path+'/alltestseqs.fasta')
    all_base_proportions = []
    all_seqs_up = []
    all_base_proportions_phreds = []
    ...
    for seqsi in range(len(dna_sequences)):
        ...
        seqs_up,aliconsus,bsquas = bsalign_alitest_0215(seqs,len(seqs))
        # seqs_up,consus = bsalign_alitest(seqs,len(seqs))
        # all_consus.append(consus)
        all_consus.append(aliconsus)
        allbs_quas.append(bsquas)
        all_seqs_up.append(seqs_up)
        if len(seqs_up)>0:
            all_ali_seqs_formaxlen.append(len(seqs_up[0]))
    sequence_length = max(all_ali_seqs_formaxlen)
    ...
    return all_base_proportions,all_consus,allbs_quas,all_consusori,dna_sequences,all_seqs_up
