...
df = pd.read_excel('plot.xlsx', sheet_name='t3_confidence')
...
ax1 = sns.barplot(x='rate', y='confidence', orient='h', data=df[0:5], ax=axes[0],width=0.5,palette="muted")
...
axes[0].set_xlabel('recognition rate', fontsize=font_size)
axes[0].set_ylabel('confidence', fontsize=font_size)


# ax2 = sns.barplot(x='data', y='success', hue="method", data=df[4:8], ax=axes[1])
# ax2.set_ylim(0.83,1.01)
ax2 = sns.barplot(x='rate', y='confidence', orient='h', data=df[5:10], ax=axes[1],width=0.5,palette="muted")
# ax2.set_xlim(0.83,1.01)

axes[1].tick_params(axis='x', labelsize=font_size)
axes[1].tick_params(axis='y', labelsize=font_size)
axes[1].set_xlabel('sensitivity rate', fontsize=font_size)
axes[1].set_ylabel('confidence', fontsize=font_size)
