...
import numpy as np
import torch
from .config import tgt_vocab_size
...
def getEdit(model,enc_inputs,dec_inputs):
    ...
    allpredict = (allpredict + allpredict1)
...
def train(loader,model):
    ...
    torch.save(model.state_dict(), 'modelsecond.pth')
    return losses
...
def testnet(model,enc_inputstest, dnatest_phreds,testseqs,testphreds):
    ...
    for i in range(len(enc_inputstest)):
        ...
        for j in range(len(prewith_phred_pulsdel)):
            ...
            score+=str(np.max(prewith_phred_pulsdel[j]))+' '
        # print(f'prewith_phred_pulsdel:{prewith_phred_pulsdel}')
        # score += ''.join(probabilities_to_quality_scores(prewith_phred_pulsdel))
        phred_scores.append(score[:-1])
        score = ''
    ...
    # print(f'score:{phred_scores}')
    return pre_seqs,phred_scores,prewith_seqs,phredwith_scores


def probabilities_to_quality_scores(probabilities):
    quality_scores = []

    for prob1 in probabilities:
        prob = np.max(prob1)
        # 计算错误概率P
        P = 1 - prob
        # 计算质量分数Q
        Q = -10 * (math.log10(P))
        # 将质量分数转换为ASCII字符
        quality_scores.append(chr(int(Q + 33)))
