...
class DNAFountain():
    ...
    def __init__(self, file_in, file_size, chunk_size, alpha, max_repeat,
            ...
            np = False,max_homopolymer = 3,gc = 0.05,crc_num=0):
    ...
    def droplet(self):
        ...
        return Droplet(data = data, seed = self.seed, rs = self.rs,
                    rs_obj = self.rs_obj, num_chunks = num_chunks, degree = d,crc_num=self.crc_num)
...
class Encode():
    def __init__(self, file_in, out = None, size = 128, max_homopolymer = 4, gc = 0.2,
                 rs = 0, delta = 0.05, c_dist = 0.1, stop = None, alpha = 0.07, no_fasta = False, rand_numpy = False,crc_num=0):
    # def __init__(self, file_in, out = None, size = 128, max_homopolymer = 4, gc = 0.2,
    #              rs = 0, delta = 0.5, c_dist = 0.03, stop = None, alpha = 0.07, no_fasta = False, rand_numpy = False):
    ...
    def main(self):
        ...
        with open(self.file_in,'rb') as f:
            file_in = f.read()
        str_list = [bin_to_str_list[i] for i in file_in]
        binstring = ''.join(str_list)
        binstring = self.file_binary_to_random(binstring)
        file_in = bytes(int(binstring[i:i + 8], 2) for i in range(0, len(binstring), 8))
        ...
        f = DNAFountain(file_in = f_in, file_size = file_size, chunk_size = self.size, max_repeat = self.max_homopolymer,
                        ...
                        np = self.rand_numpy, alpha = self.alpha, stop = self.stop, crc_num = self.crc_num)
