...
def decode_phred(dna_sequences,index_length,total_count,phreds):
    bit_segments = []
    print(f"共有{len(dna_sequences)}条序列，期望能解码出{total_count}行bits")
    bit_phreds = []
    for sequence_index, dna_sequence in enumerate(dna_sequences):
        ...
        bit_phreds.append(phreds[sequence_index])
        bit_phreds.append(phreds[sequence_index])
    ...
    for bit_segment in bit_segments:
        ...
        if segment_index < total_count:
            ...
            remain_bit_phreds.append(bit_phreds[sj])
        sj += 1
    print(f"bit_segments:{len(bit_segments)},len(remain_bit_segments):{len(remain_bit_segments)}")
    return remain_bit_segments,remain_bit_phreds
...
def yinyangdecode(dna_sequences,index_length,total_count,phreds):
    if len(phreds)>0:
        bit_segments,bit_phreds = decode_phred(dna_sequences, index_length, total_count,phreds)
        bit_segments = [''.join(map(str, numbers)) for numbers in bit_segments]
        return bit_segments,bit_phreds
    else:
